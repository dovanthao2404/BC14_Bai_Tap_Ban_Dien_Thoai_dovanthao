...
const delegationListPhoneView = (e) => {
  if (e.target.getAttribute("data-action") === "add") {
    ...
    let notifiSucces = document.querySelector(".notifiSuccess")
    if (notifiSucces.style.display === "block") {
      notifiSucces.style.display = "none";
    }
    notifiSucces.style.display = "block";
    setTimeout(() => {
      notifiSucces.style.display = "none";
    }, 3000)
